...
public class PrincipalAction extends SigaActionSupport {
	...
	// Nem ser√° necess√°rio herdar de Selecao
	...
	public String aSelecionar() throws Exception {
		try {
			...
			// alterada a condiÁ„o que verifica se È uma solicitaÁ„o do siga-sr
			// dessa forma a regex verifica se a sigla comeÁa com SR ou sr e
			// termina com n˙meros
			// necess·rio para n„o dar conflito caso exista uma lotaÁ„o que
			// inicie com SR
			...
			if (response.length == 1 && Integer.valueOf(response[0]) == 0) {
				// verificar se apÛs a retirada dos prefixos referente
				// ao org„o (sigla_orgao_usu = RJ ou acronimo_orgao_usu = JFRJ)
				// e n„o achar resultado com as opÁıes anteriores
