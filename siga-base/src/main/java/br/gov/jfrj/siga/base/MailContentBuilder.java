...
public class MailContentBuilder {
	 ...
	 */
	public Multipart build(String messageText, String messageHtml, List<URL> messageHtmlInline, List<URL> attachments) throws MessagingException, UnsupportedEncodingException {
	...
	private void addTextVersion(Multipart mpRelatedAlternative, String messageText) throws MessagingException, UnsupportedEncodingException {		
		...
		MimeBodyPart textPart = new MimeBodyPart(ihs, Base64.encode(messageText.getBytes("utf-8")));
		mpRelatedAlternative.addBodyPart(textPart);	
	}

	private void addHtmlVersion(Multipart parent, String messageHtml, List<URL> embeded) throws MessagingException, UnsupportedEncodingException {
		...
		if (messageHtml != null) {
			...
			// Html
			InternetHeaders ihs = new InternetHeaders();
			ihs.addHeader("Content-Type", "text/html; charset=UTF-8");
			ihs.addHeader("Content-Transfer-Encoding", "base64");
			
			HashMap<String, String> cids = new HashMap<String, String>();
			String newMessageHtml = replaceUrlWithCids(messageHtml, cids);
			final MimeBodyPart htmlPart = new MimeBodyPart(ihs, Base64.encode(newMessageHtml.getBytes("utf-8")));			
	...
	private void addHtmlVersion(Multipart parent, String messageHtml) throws MessagingException, UnsupportedEncodingException {
		...
		if (messageHtml != null) {
			...
			// Adding image header to html body
			messageHtml = addImageHeaderHtmlMessage(messageHtml);
			List<URL> embeded = new ArrayList<URL>();
			embeded.add(MailContentBuilder.class.getResource("/cabecalhoEmail.png"));
	...
	private String addImageHeaderHtmlMessage(String html){
		
		int indInicialTagBody = html.indexOf("body");
		
		String newHtmlContent = html.substring(0, indInicialTagBody+5);
		
		newHtmlContent += "<img src=\"cabecalhoEmail.png\"/><br/>";
		
		newHtmlContent += html.substring(indInicialTagBody+5, html.length());
