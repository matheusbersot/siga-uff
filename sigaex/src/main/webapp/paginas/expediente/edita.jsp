					...
					<table class="gt-form-table">
						...
						<c:if test='${tipoDocumento == "externo"}'>
							<tr>
								...
								<td colspan="3"><ww:textfield name="dtDocOriginalString"
										size="10" onblur="javascript:verifica_data(this, true);" /></td>
							</tr>
							<tr>
								<td>Nº original:</td>
								<input type="hidden" name="campos" value="numExtDoc" />
								...
								<td><siga:selecao propriedade="cpOrgao" tema="simple"
										modulo="siga" /></td>
							</tr>
							<tr>
							...
							<c:choose>
								...
								<c:otherwise>
									...
									<td colspan="3"><siga:selecao propriedade="subscritor"
											modulo="siga" tema="simple" />&nbsp;&nbsp;<ww:checkbox
											name="substituicao"
						...
						<tr>
							<td>Função;<wbr />Lotação;<wbr />Localidade:
							</td>
...
--%>
					...
					--%>
						...
						<c:if test='${ tipoDocumento == "externo" }'>
							<ww:hidden name="idFormaDoc"
								value="%{formaDocPorTipo.idFormaDoc}" />
					...
					<ww:if test="%{modelos.size > 1}">
						...
						<tr
							style="display:<ww:if test="%{modelo.descricaoAutomatica}">none</ww:if><ww:else>visible</ww:else>">
...
--%>
							...
							<c:if
								...
								<tr>
									<td colspan="4"><siga:span id="spanEntrevista"
											...
											<c:if
												test="${modelo.conteudoTpBlob == 'template/freemarker'}">
						...
						<tr>
							<td colspan="4"><input type="button"
								onclick="javascript: gravarDoc();" name="gravar" value="Ok"
								class="gt-btn-small gt-btn-left" /> <c:if
									test='${tipoDocumento == "interno"}'>
									<c:if
										test="${not empty modelo.nmArqMod or modelo.conteudoTpBlob == 'template/freemarker'}">
										...
										<input type="button" name="ver_doc_pdf"
											onclick="javascript: popitup_documento(true);"
											value="Visualizar a Impressão"
											class="gt-btn-large gt-btn-left" />
									</c:if>
								</c:if></td>
						</tr>
